...
func TestSettingsAPIRoutesUseApplicationService(t *testing.T) {
	service := &fakeSettingsService{value: settingsapp.Settings{AppName: "即应", OrganizationName: "长亭科技", ContactDirectoryMode: settingsapp.ContactDirectoryModeOrganization, AllowUserNicknameEditing: true}}
	...
	request = httptest.NewRequest(http.MethodPut, "/api/admin/settings/info", bytes.NewBufferString(`{"app_name":"星环协作","organization_name":"长亭科技企业安全","contact_directory_mode":"friends","allow_user_nickname_editing":false}`))
	...
	if service.updateCommand.AppName != "星环协作" || service.updateCommand.OrganizationName != "长亭科技企业安全" || service.updateCommand.ContactDirectoryMode != settingsapp.ContactDirectoryModeFriends || service.updateCommand.AllowUserNicknameEditing == nil || *service.updateCommand.AllowUserNicknameEditing {
	...
	if data["app_name"] != "星环协作" || data["allow_user_nickname_editing"] != false {
...
func (s *fakeSettingsService) Update(_ context.Context, cmd settingsapp.UpdateCommand) (settingsapp.Settings, error) {
	...
	if cmd.AllowUserNicknameEditing != nil {
		s.value.AllowUserNicknameEditing = *cmd.AllowUserNicknameEditing
	}
	return s.value, nil
}
