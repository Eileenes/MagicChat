...
const frameSamples = (asrTargetSampleRate * asrFrameDurationMS) / 1_000
...
export class PCM16StreamEncoder {
  ...
  constructor(sourceSampleRate: number) {
    if (!Number.isFinite(sourceSampleRate) || sourceSampleRate < asrTargetSampleRate) {
  ...
  push(samples: Float32Array): ArrayBuffer[] {
    ...
    while (this.sourcePosition + 1 < source.length) {
      ...
      const value = source[index]! * (1 - fraction) + source[index + 1]! * fraction
  ...
  flush(): ArrayBuffer[] {
    ...
    if (this.pendingSamples.length > 0) frames.push(encodePCM16(this.pendingSamples))
    this.pendingSamples = []
  ...
  private takeFrames() {
    ...
    while (this.pendingSamples.length >= frameSamples) {
      frames.push(encodePCM16(this.pendingSamples.slice(0, frameSamples)))
      this.pendingSamples = this.pendingSamples.slice(frameSamples)
    }
    return frames
  }
}
