...
describe("ClientBrandMetadata", () => {
  ...
  it("uses the configured app name in document metadata", async () => {
    ...
    await waitFor(() => {
      ...
      expect(getMetaContent('meta[name="twitter:title"]')).toBe("有事｜星环协作")
      expect(getMetaContent('meta[name="description"]')).toBe("AI时代下的团队协作入口")
      expect(getMetaContent('meta[property="og:description"]')).toBe("AI时代下的团队协作入口")
      expect(getMetaContent('meta[name="twitter:description"]')).toBe("AI时代下的团队协作入口")
