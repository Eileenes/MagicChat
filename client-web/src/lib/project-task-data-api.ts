...
export async function listClientProjectTaskActivities(
  ...
  const payload = await readJson<
    | ProjectTaskDataErrorEnvelope
    | ProjectTaskDataSuccessEnvelope<ProjectTaskActivityListResponse>
  >(response)
...
export async function addClientProjectTaskComment(
  ...
  const payload = await readJson<
    | ProjectTaskDataErrorEnvelope
    | ProjectTaskDataSuccessEnvelope<ProjectTaskActivityResponse>
  >(response)
  ...
  return normalizeProjectTaskActivity(
    (
      payload as
        ProjectTaskDataSuccessEnvelope<ProjectTaskActivityResponse> | undefined
    )?.data
  )
}
...
function isProjectTaskActivityType(
  value: unknown
): value is ProjectTaskActivity["type"] {
  return value === "created" || value === "updated" || value === "commented"
}

function isProjectTaskActivityChange(
  value: unknown
): value is ProjectTaskActivityChange {
  return (
    typeof value === "object" &&
    value !== null &&
